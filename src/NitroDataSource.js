...
export default class NitroDataSource {
  constructor (domain) {
    ...
    const authLink = setContext((_, { headers }) => {
      // token to use for accessing the graphql api
  ...
  async getOutboxCollectionPage (actorId) {
    ...
    if (result.data) {
      ...
      await Promise.all(
        posts.map((post) => {
          const article = createArticleActivity(post.content, slug, post.id, post.createdAt)
          delete article['@context']
          outboxCollection.orderedItems.push(article)
        })
      )
